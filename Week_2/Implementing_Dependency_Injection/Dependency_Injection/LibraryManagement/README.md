...
This exercise's three steps were largely already satisfied by the
setup above; here's exactly what changed to make the injection
**observable and verifiable**, not just functionally present:

1. **XML wiring** (`applicationContext.xml`) — unchanged; already wires
   `bookRepository` into `bookService` via `<property ref="...">`, as
   required.
2. **`BookService` setter** — `setBookRepository(...)` now logs when
   it's called (`"Dependency injected: BookRepository -> BookService"`),
   so you can watch it fire during context startup in the console
   output, before `main()`'s own code runs. A `getBookRepository()`
   getter was also added, purely so the injected value can be checked
   directly rather than only inferred from `getAllBooks()` working.
3. **Verification in `main()`** — before calling `getAllBooks()`, the
   main class now explicitly checks
   `bookService.getBookRepository() != null` and logs whether injection
   succeeded or failed. The test suite adds
   `bookRepositoryIsInjectedIntoBookService()`, which goes one step
   further than a null check: it confirms `BookService` is holding the
   *exact same* `BookRepository` bean instance the container registered
   under the id `bookRepository` — proof that Spring did the wiring,
   rather than `BookService` having quietly constructed its own
   `BookRepository` somewhere.

Run it the same way as before (`mvn compile exec:java`); you'll now see
the injection log line appear during context startup, followed by the
explicit verification line, before the book list is printed:
```
[main] INFO com.library.LibraryManagementApplication - START - loading applicationContext.xml
[main] INFO com.library.service.BookService - Dependency injected: BookRepository -> BookService
[main] INFO com.library.LibraryManagementApplication - Dependency Injection verified: BookService.bookRepository is populated
[main] INFO com.library.LibraryManagementApplication - Books retrieved via BookService -> BookRepository: [The Hobbit, 1984, Clean Code, Effective Java]
[main] INFO com.library.LibraryManagementApplication - END - context closed
```
